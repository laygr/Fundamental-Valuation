...
let freeCashFlow
    ...
    deltaFixedAssetsAtCost
    (taxRate:D)
    (interestPaymentOnDebt:D)
    (``interest earned on cash and marketable securities``:D)
    =
    profitAfterTax
...
let simulation (values0:DV) =
    ...
    let equityValues = [| for _ in 1 .. 10000 -> enterpriseValue5Years (values()) |> float |]
